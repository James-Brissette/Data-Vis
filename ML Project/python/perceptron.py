        ...
        if (counter % 250 == 0):
            ...
            for att in w:
                if np.abs(w[att]) > 1e-3:
                    temp[att] = w[att]
            w = temp
            print('        Example ' + str(counter) + ' completed -- weight vector has ' + str(len(w)) + ' elements')
...
def average_train(epochs, rate, training_set, dev_set, mistakes, testData, cutoff):
    ...
    predictAndLabel(test,tracker[idx][1][0],tracker[idx][1][1])
...
def simple_train(epochs,rate,training_set,dev_set,mistakes,testData, evalSet, cutoff):
    ...
    idx = list(tracker.values()).index(max(tracker.values()))
#    predictAndLabel(evalSet,tracker[idx][1][0],tracker[idx][1][1])
    print("Max accuracy on Dev set: " + str(tracker[idx][0]) + ' at epoch ' + str(idx +1))
...
def predictAndLabel(data,w,b):
    ...
    for example in data:
        ...
        if (counter % 250 == 0):
            print('        Predict/Label ' + str(counter) + ' completed')
        counter = counter + 1
    return mistakes
        ...
        for key in list(example.keys()):
            if key == 'label':
                ...
                try:
                    ...
                    try:
                        w[key] = rate*((example[key]/(1+np.exp(-ywTx)*example[key])))
                    except:
                        pass
        try:           
...
#        print('Epoch: ' + str(i + 1) + ' of ' + str(epochs))
        ...
        np.random.shuffle(dev_set)
        mistakes = predictLogistic(dev_set,w,b)
...
#    plt.plot(np.arange(1,epochs+1,1),[i[0] for i in tracker.values()])
    ...
    idx = list(tracker.values()).index(max(tracker.values()))
    print("Accuracy on Dev set: " + str(tracker[idx][0]) + ' at epoch ' + str(idx +1))
    
    np.random.shuffle(test_set)
    mistakes = predictLogistic(test_set,tracker[idx][1][0],tracker[idx][1][1])
...
#average_train(1,.01,data[0:20000],data[20000:25000],mistakes,testData, test, cutoff)
...
#tracker = simple_train(20,.01,trimmedData,data[20000:25000], mistakes,testData,cutoff)
#accuracy = average_cross_validate(1, trimmedData)
#accuracy = average_cross_validate(30, trimmedData)
tracker = logisticRegression_train(1,.1,w,b, 1000, trimmedData[0:20000], data[20000:25000], mistakes, test_set)
#tracker = average_train(20,1,trimmedData[0:20000],data[20000:25000], mistakes,testData,cutoff)
